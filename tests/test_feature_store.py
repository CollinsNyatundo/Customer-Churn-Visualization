...
def sample_featured_df(sample_client_df, sample_price_df):
    """
    Fully-featured DataFrame built from conftest.py's synthetic fixtures.

    Uses sample_client_df/sample_price_df (in-memory, no disk I/O) rather
    than BCGClientSource().load(), which reads data/raw/*.csv — a path
    that's gitignored and never generated by test infrastructure. Reading
    from disk here broke every fresh clone and every CI run silently,
    since CI never generates that CSV before running pytest.
    """
    ...
    price_agg = aggregate_price_data(sample_price_df)
