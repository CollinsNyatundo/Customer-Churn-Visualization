...
def churn_by_tenure(df: pd.DataFrame) -> go.Figure:
    ...
    df["tenure_band"] = pd.cut(
        df["num_years_antig"], bins=[0, 1, 3, 5, 10, 100], labels=["<1yr", "1-3yr", "3-5yr", "5-10yr", "10+yr"]
    )
...
def churn_by_category(df: pd.DataFrame, col: str, title: str | None = None) -> go.Figure:
    rates = df.groupby(col)["churn"].mean().mul(100).round(1).sort_values().reset_index()
...
def churn_by_nps(df: pd.DataFrame) -> go.Figure:
    ...
    df = df.copy()
    df["nps_band"] = pd.cut(
        df["nps_score"], bins=[-101, -30, 0, 30, 101], labels=["Detractor", "Passive-", "Passive+", "Promoter"]
    )
...
def churn_by_support_tickets(df: pd.DataFrame) -> go.Figure:
    if "num_tickets_6m" not in df.columns:
        return go.Figure()
    df = df.copy()
    df["ticket_band"] = pd.cut(df["num_tickets_6m"], bins=[-1, 0, 2, 5, 100], labels=["None", "1-2", "3-5", "6+"])
...
def margin_vs_churn(df: pd.DataFrame) -> go.Figure:
    ...
    return px.scatter(
        ...
        color_discrete_map={"Churned": _C["churned"], "Retained": _C["retained"]},
        opacity=0.5,
        title="Net margin vs consumption by churn",
...
def correlation_heatmap(df: pd.DataFrame, top_n: int = 15) -> go.Figure:
    ...
    fig.update_layout(title=f"Correlation heatmap (top {top_n} vs churn)", template=_T, height=520)
    return fig
