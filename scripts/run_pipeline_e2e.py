...
def plot_roc(models_results: dict, path: Path) -> None:
    ...
    for (name, res), col in zip(models_results.items(), colors):
        ...
        ax.plot(fpr, tpr, color=col, lw=2, label=f"{name} (AUC={res['test_auc']:.3f})")
...
def plot_calibration(models_results: dict, path: Path) -> None:
    ...
    for (name, res), col in zip(models_results.items(), colors):
        ...
        ax.plot(mean_pred, frac_pos, "o-", color=col, lw=2, label=f"{name} (Brier={res['brier']:.3f})")
    ax.set(xlabel="Mean predicted probability", ylabel="Fraction of positives", title="Calibration Curves")
...
def plot_shap(pipeline: Pipeline, X_sample: pd.DataFrame, available_num: list, available_cat: list, path: Path) -> None:
    pre = pipeline.named_steps["pre"]
    clf = pipeline.named_steps["clf"]
    X_t = pre.transform(X_sample)

    try:
        cat_names = pre.named_transformers_["cat"].named_steps["ohe"].get_feature_names_out(available_cat).tolist()
    except Exception:
        cat_names = []
    ...
    ax.set(title="Top 15 Features — Mean |SHAP value|", xlabel="Mean absolute SHAP value")
    fig.tight_layout()
    fig.savefig(path, dpi=120, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    print("      Sources active    : BCG client + BCG price + CRM (syn) + Support (syn) + Billing (syn)")
            ...
            mlflow.log_params(
                {
                    ...
                    **{f"clf_{k}": str(v) for k, v in list(clf.get_params().items())[:8]},
                }
            )
            ...
            mlflow.log_metrics(
                {
                    ...
                    **{f"pipe_meta_{k}": float(v) for k, v in pipe_meta.items() if isinstance(v, (int, float))},
                }
            )
    ...
    if best_name in ("GradientBoosting", "RandomForest"):
        ...
        try:
            feat_names, shap_vals = plot_shap(best_pipe, sample, avail_num, avail_cat, shap_path)
    ...
    for name, res in results.items():
        ...
        print(
            f"  {name:<22} {res['cv_auc']:>8.4f} {res['test_auc']:>9.4f} "
    ...
    for label, lname in [(0, "Retained"), (1, "Churned (positive)")]:
        r = rpt[str(label)]
        print(
            f"  {lname:<20} {r['precision']:>10.4f} {r['recall']:>8.4f} "
            f"{r['f1-score']:>8.4f} {int(r['support']):>9,}"
        )
    print(f"  {'Accuracy':<20} {'':>10} {'':>8} {rpt['accuracy']:>8.4f} " f"{int(rpt['macro avg']['support']):>9,}")
